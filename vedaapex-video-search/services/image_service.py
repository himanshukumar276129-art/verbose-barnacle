...
class ImageSearchService:
    ...
    async def search(
        ...
        logger.info(f"Searching images: {query} (page {page}, size {page_size})")

        # Check cache
        cached_result = None
        if use_cache:
            cached_result = await cache_service.get_search_results(
                query, page, search_type="image"
            )
        ...
        # Fetch from provider
        try:
            raw_results = await self.provider.search_images(
                query=query,
                page=page,
                per_page=page_size,
                sort=sort,
            ...
            await cache_service.set_search_results(
                query, page, normalized, search_type="image"
            )
    ...
    def _normalize_response(
        ...
        # Extract images
        photos = raw_response.get("photos", [])
        for photo in photos:
            try:
                image_data = helpers.extract_image_metadata(photo)
