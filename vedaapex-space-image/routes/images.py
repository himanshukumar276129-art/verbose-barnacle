...
image_service = ImageSearchService(provider_manager)
...
async def search_images(
    ...
    """
    Search images from multiple providers (NASA, Wikimedia, Pexels).
    
    Provider selection based on query type:
    - Space queries (mars, moon, etc) → NASA first
    - Scientific queries (cancer cell, etc) → Wikimedia first  
    ...
    **Example:**
    ```
    GET /api/v1/images/search?q=mars%20rover&page=1&page_size=20
    ```
    """
    try:
